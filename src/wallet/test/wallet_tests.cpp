...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup) {
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetBalance().m_mine_immature, 2 * SUBSIDY);
    }
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(result.last_scanned_block, newTip->GetBlockHash());
        BOOST_CHECK_EQUAL(*result.last_scanned_height, newTip->nHeight);
        BOOST_CHECK_EQUAL(wallet.GetBalance().m_mine_immature, SUBSIDY);
    }
...
BOOST_FIXTURE_TEST_CASE(importmulti_rescan, TestChain100Setup) {
    ...
    // after.
    {
        std::shared_ptr<CWallet> wallet = std::make_shared<CWallet>(
            chain.get(), "", CreateDummyWalletDatabase());
        ...
        BOOST_CHECK_EQUAL(
            ...
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":"
                      ...
                      "pruning or data corruption (see lotusd log for "
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain100Setup) {
    ...
    BOOST_CHECK_EQUAL(wtx.GetImmatureCredit(), SUBSIDY);
}
...
public:
    ListCoinsTestingSetup() {
        CreateAndProcessBlock({},
                              GetScriptForRawPubKey(coinbaseKey.GetPubKey()));
        wallet = std::make_unique<CWallet>(m_chain.get(), "",
...
BOOST_FIXTURE_TEST_CASE(ListCoins, ListCoinsTestingSetup) {
    ...
    BOOST_CHECK_EQUAL(SUBSIDY, wallet->GetAvailableBalance());
...
BOOST_FIXTURE_TEST_CASE(wallet_disableprivkeys, TestChain100Setup) {
    ...
    std::shared_ptr<CWallet> wallet =
        std::make_shared<CWallet>(chain.get(), "", CreateDummyWalletDatabase());
    wallet->SetupLegacyScriptPubKeyMan();
